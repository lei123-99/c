...
x_urls = []
for url in urls:  # 瀵箄rls杩涜澶勭悊锛宨p绗洓浣嶄慨鏀逛负1锛屽苟鍘婚噸
    url = url.strip()
    ip_start_index = url.find("//") + 2
    ip_end_index = url.find(":", ip_start_index)
    ...
    x_urls.append(x_url)
urls = set(x_urls)  # 鍘婚噸寰楀埌鍞竴鐨刄RL鍒楄〃
...
for url in valid_urls:
    print(url)
# 閬嶅巻缃戝潃鍒楄〃锛岃幏鍙朖SON鏂囦欢骞惰В鏋�
for url in valid_urls:
    try:
        # 鍙戦€丟ET璇锋眰鑾峰彇JSON鏂囦欢锛岃缃秴鏃舵椂闂翠负0.5绉�
        ip_start_index = url.find("//") + 2
        ip_dot_start = url.find(".") + 1
            ...
            for item in json_data['data']:
                if isinstance(item, dict):
                    ...
                    if name and urlx:
                        ...
                        name = name.replace("涓ぎ", "CCTV")
                        name = name.replace("澶", "")
                        name = name.replace("楂樻竻", "")                        
                        name = name.replace("HD", "")
                        name = name.replace("鏍囨竻", "")
                        name = name.replace("棰戦亾", "")
                        name = name.replace("-", "")
                        name = name.replace(" ", "")
                        ...
                        name = name.replace("(", "")
                        name = name.replace(")", "")
                        name = re.sub(r"CCTV(\d+)鍙�", r"CCTV\1", name)
                        name = name.replace("CCTV1缁煎悎", "CCTV1")
                        name = name.replace("CCTV2璐㈢粡", "CCTV2")
                        name = name.replace("CCTV3缁艰壓", "CCTV3")
                        name = name.replace("CCTV4鍥介檯", "CCTV4")
                        name = name.replace("CCTV4涓枃鍥介檯", "CCTV4")                        
                        name = name.replace("CCTV5浣撹偛", "CCTV5")
                        name = name.replace("CCTV6鐢靛奖", "CCTV6")
                        name = name.replace("CCTV7鍐涗簨", "CCTV7")
                        name = name.replace("CCTV7鍐涘啘", "CCTV7")
                        name = name.replace("CCTV7鍐滀笟", "CCTV7")
                        name = name.replace("CCTV7鍥介槻鍐涗簨", "CCTV7")
                        name = name.replace("CCTV8鐢佃鍓�", "CCTV8")
                        name = name.replace("CCTV9璁板綍", "CCTV9")
                        name = name.replace("CCTV9绾綍", "CCTV9")
                        name = name.replace("CCTV10绉戞暀", "CCTV10")
                        name = name.replace("CCTV11鎴忔洸", "CCTV11")
                        name = name.replace("CCTV12绀句細涓庢硶", "CCTV12")
                        name = name.replace("CCTV13鏂伴椈", "CCTV13")
                        name = name.replace("CCTV鏂伴椈", "CCTV13")
                        name = name.replace("CCTV14灏戝効", "CCTV14")
                        name = name.replace("CCTV15闊充箰", "CCTV15")
                        name = name.replace("CCTV16濂ユ灄鍖瑰厠", "CCTV16")
                        name = name.replace("CCTV17鍐滀笟鍐滄潙", "CCTV17")
                        name = name.replace("CCTV17鍐滀笟", "CCTV17")
                        name = name.replace("CCTV5+浣撹偛璧涜", "CCTV5+")
                        name = name.replace("CCTV5+浣撹偛璧涗簨", "CCTV5+")
                        name = name.replace("CCTV5+浣撹偛", "CCTV5+")
...
with open("iptv.txt", 'w', encoding='utf-8') as file:
    file.write('澶棰戦亾,#genre#\n')
    for result in results:
        channel_name, channel_url = result.split(',',1)
        if 'CCTV' in channel_name or 'CHC' in channel_name:
            file.write(f"{channel_name},{channel_url}\n")
    file.write('鍗棰戦亾,#genre#\n')
    for result in results:
        channel_name, channel_url = result.split(',',1)
        if '鍗' in channel_name:
            file.write(f"{channel_name},{channel_url}\n")
    file.write('鍏朵粬棰戦亾,#genre#\n')
    for result in results:
        channel_name, channel_url = result.split(',',1)
        if 'CCTV' not in channel_name and '鍗' not in channel_name and '娴嬭瘯' not in channel_name and 'CHC' not in channel_name:
            file.write(f"{channel_name},{channel_url}\n")
